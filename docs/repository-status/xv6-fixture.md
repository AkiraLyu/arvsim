...
测试依赖 `tests/support` 中的平台实现、外部 xv6 内核和文件系统镜像，以及固定的启动和用户程序输出。完整测试最多允许执行 20 亿步，并依赖 CPU 中的 xv6 加速才能在可接受时间内完成。2026-07-15 的具体命令和结果见 [`verification.md`](./verification.md)。
