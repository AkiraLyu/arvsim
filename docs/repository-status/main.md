...
- 显式 `--uart-base` 与 `--platform bare` 冲突时会报参数错误；数值和大小解析错误会指出对应选项，帮助文本列出全部大小后缀。
...
- UART 与 DRAM 区域重叠、地址溢出、入口超出 DRAM、入口未按 2 字节对齐，以及初始栈对齐后落到 DRAM 之外都会在组装时被拒绝。
...
- `bare` 和 `uart` 仍是轻量平台，不包含 CLINT、PLIC 或 virtio；完整 `VirtPlatform` 已在库中实现，但主命令行尚未暴露对应 preset 和块介质参数。
