...
执行日期：2026-07-15。基线为提交 `256d787`，同时包含当前工作区中的 P0 修复；以下结果对应文档描述的当前代码。
...
- `src/main.rs`：3 个通过。
- `tests/cli.rs`：3 个通过。
- `tests/rv64i_smoke.rs`：4 个通过。
- `tests/xv6_fixture.rs`：1 个通过，4 个未执行。
- 合计：59 个通过，4 个未执行，没有失败。
...
- `cargo test --release --test xv6_fixture xv6_runs_quick_usertests -- --ignored`：通过，输出 `ALL TESTS PASSED`，耗时 241 秒。
